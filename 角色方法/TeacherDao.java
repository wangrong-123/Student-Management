...
public class TeacherDao {
	...
	public void add(Teacher teacher) throws SQLException {
		...
		String sql = "insert into teacher(tName,tuserName,pwd) values(?,?,?)";
		queryRunner.update(sql, teacher.gettName(), teacher.getTuserName(), teacher.getPwd());
	}
	...
	public void update(Teacher teacher) throws SQLException {
		QueryRunner queryRunner = new QueryRunner(PropertiesUtils.getDataSource());
		String sql = "update teacher set tName = ?,tuserName = ? where tId = ?";
		queryRunner.update(sql, teacher.gettName(), teacher.getTuserName(), teacher.gettId());
	...
	public PageInfo<Teacher> list(Teacher teacher, PageInfo<Teacher> pageInfo) throws SQLException {
		...
		if (teacher != null && StringUtils.isNoneBlank(teacher.getTuserName())) {
			_sql += " and TUSERNAME like ?";
			_list.add("%" + teacher.getTuserName() + "%");
		}
		// list转化数组
		...
		String sql = "select * from teacher where 1=1 " + _sql + " limit "
				+ (pageInfo.getPageNo() - 1) * pageInfo.getPageSize() + " , " + pageInfo.getPageSize();
		System.out.println("调用了TeacherDao方法！");
	...
	// 登录
	public Teacher login(String tuserName, String pwd) throws SQLException {
		QueryRunner queryRunner = new QueryRunner(PropertiesUtils.getDataSource());
		String sql = "select * from teacher where tuserName = ? and pwd = ?";
		Teacher teacher = queryRunner.query(sql, new BeanHandler<>(Teacher.class), tuserName, pwd);
		return teacher;
	}
